...
# Make the embedded ``tmhp`` package importable regardless of which virtual
# environment runs this script.
_script_dir = os.path.dirname(os.path.abspath(__file__))
_docs_dir = os.path.dirname(_script_dir)
_repo_root = os.path.dirname(_docs_dir)
_tmhp_src = os.path.join(_repo_root, "src", "enex_analysis", "tmhp", "src")
if _tmhp_src not in sys.path:
    sys.path.insert(0, _tmhp_src)

import tmhp.calc_util as cu  # noqa: E402
from tmhp.mollier_diagram import REF_LIMITS  # noqa: E402
from tmhp.refrigerant import calc_ref_state  # noqa: E402
...
def solve_cycle(
    ...
    # The source/sink bound temperatures are intentionally omitted: the widget
    # recovers them from the active T_source / T_sink slider indices.
    return pts


def main() -> None:
    out_dir = os.path.join(_docs_dir, "source", "_static", "widgets")
