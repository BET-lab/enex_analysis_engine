...
# --- 1. 시뮬레이션 설정 ---
H = 0.8; D = 0.3; N = 5
tank = StratifiedTankTDMA(r0=0.25, x_shell=0.005, x_ins=0.05, k_shell=50, k_ins=0.04, H=H, N=N, h_w=10, h_o=5)

T_initial_C = 40.0 # 초기 탱크 온도 [°C]
T_in_C = 15.0 # 유입수 온도 [°C]
heater_power_W = 45 # 히터 전력 [W]
heater_node = 1 # 히터 위치
...
dt = 60 # 시간 간격 (60초 = 1분)
sim_hours = 24
n_steps = int(sim_hours * cu.h2s / dt)
t_span = np.arange(n_steps) * dt

# --- 2. 24시간 가상 스케줄 생성 ---
# 외기 온도 스케줄 (15°C ~ 25°C 사이에서 변동)
T_amb_C = 20 - 5 * np.cos(2 * np.pi * t_span / (24 * cu.h2s))
T_amb_K = cu.C2K(T_amb_C)
...
morning_usage_L = 100
evening_usage_L = 100
morning_start_idx = int(7 * cu.h2s / dt)
morning_end_idx = int(9 * cu.h2s / dt)
evening_start_idx = int(19 * cu.h2s / dt)
evening_end_idx = int(21 * cu.h2s / dt)

# 사용 시간에 걸쳐 유량 균등 분배
dV_schedule[morning_start_idx:morning_end_idx] = (morning_usage_L * cu.L2m3) / (morning_end_idx - morning_start_idx) / dt
dV_schedule[evening_start_idx:evening_end_idx] = (evening_usage_L * cu.L2m3) / (evening_end_idx - evening_start_idx) / dt

# --- 3. 시뮬레이션 실행 ---
# 결과 저장을 위한 배열 초기화
T_results = np.zeros((n_steps, N))
T_current = np.full(N, cu.C2K(T_initial_C)) # 초기 온도를 Kelvin으로 설정

# 메인 시뮬레이션 루프
for i in range(n_steps):
    # 서모스탯 로직에 따라 히터 전원 결정
    heater_on = False
    heater_sensor_temp_C = cu.K2C(T_current[heater_node-1])
...
# --- 4. 결과 시각화 ---
fig, ax = plt.subplots(figsize=(dm.cm2in(15), dm.cm2in(7)))

t_hours = t_span / 3600
T_results_C = cu.K2C(T_results)

# 각 노드(성층)별 온도 변화 플로팅
for i in range(N):
    # 최상단(Node 1)은 굵게, 최하단(Node 10)은 점선으로 표시
    if i == 0:
        ax.plot(t_hours, T_results_C[:, i], label=f'Node {i+1} (Top)', linewidth=2.5)
    elif i == N - 1:
        ax.plot(t_hours, T_results_C[:, i], label=f'Node {i+1} (Bottom/Heater)', linestyle='--')
    else:
        ax.plot(t_hours, T_results_C[:, i], label=f'Node {i+1}')
...
ax.legend(title='Tank Layers', bbox_to_anchor=(1.02, 1), loc='upper left')

# 온수 사용 시점 시각적으로 표시
ax_twin = ax.twinx()
ax_twin.fill_between(t_hours, 0, dV_schedule * cu.h2s * 1000, color='gray', alpha=0.2, label='Hot Water Usage')
ax_twin.set_ylabel('Hot Water Usage (L/h)', fontsize=12)
